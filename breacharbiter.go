...
func (b *breachArbiter) contractObserver() {
	...
	for {
		...
		case breachInfo := <-b.breachedContracts:
			// A new channel contract has been added to the
			// retribution store! We first register for a
			// notification to be dispatched once the breach
			// transaction (the revoked commitment transaction) has
			// been confirmed in the chain to ensure we're not
