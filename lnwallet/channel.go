...
// chain.
//
// If mutateState is set to true, then the HTLCs in the view will add
// height of all added HTLCs will be set to nextHeight, and all remove
// heights of all removed HTLCs will be set to nextHeight. This should
// therefore only be set once for each height, and only in concert with
// signing a new commitment.
...
func (lc *LightningChannel) availableBalance() (lnwire.MilliSatoshi, int64) {
	...
	ourBalance, _, commitWeight, _, _ := lc.computeView(htlcView, false, false)
