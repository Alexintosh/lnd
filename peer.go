...
func (p *peer) addLink(chanPoint *wire.OutPoint,
	...
	linkCfg := htlcswitch.ChannelLinkConfig{
		...
		OnChannelFailure: func(chanID lnwire.ChannelID,
			...
			// The link has notified us about a failure. We launch
			// a go routine to stop the link, disconnect the peer
			// and optionally force close the channel. We must
			// launch a goroutine since we must let
			// OnChannelFailure return in order for the link to
			// completely stop in the call to RemoveLink.
			p.wg.Add(1)
			go func() {
				defer p.wg.Done()

				// We begin by removing the link from the
				// switch, such that it won't be attempted used
				// for any more updates.
				err := p.server.htlcSwitch.RemoveLink(chanID)
				if err != nil {
					peerLog.Errorf("unable to stop "+
						"link(%v): %v", shortChanID,
						err)
				}

				// If the error encountered was severe enough,
				// we'll now force close the channel.
				if linkErr.ForceClose {
					...
					if err != nil {
						peerLog.Errorf("unable to "+
							"force close link(%v):"+
							" %v", shortChanID, err)
					} else {
						peerLog.Infof("channel(%v) "+
							"force closed with "+
							"txid %v", shortChanID,
							closeTx.TxHash())
					}
				...
				if linkErr.ShouldSendToPeer() {
					// If SendData is set, send it to the
					// peer. If not, we'll use the standard
					// error messages in the payload. We
					// only include sendData in the cases
					// where the error data does not
